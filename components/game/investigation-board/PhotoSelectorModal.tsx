...
export function PhotoSelectorModal({ isOpen, onClose, unlockedContent, caseId }: PhotoSelectorModalProps) {
  const [photos, setPhotos] = useState<PhotoItem[]>([])
  const [photoFilter, setPhotoFilter] = useState<string>('all')
  const [showAllFilters, setShowAllFilters] = useState(false)

  // Load photos from metadata
  useEffect(() => {
    if (!isOpen) return
  ...
  const handleDragStart = (photo: PhotoItem, e: React.DragEvent) => {
            ...
            <div className="space-y-3">
              {filteredPhotos.map((photo) => (
                <PhotoCard
                  key={photo.id}
                  photo={photo}
                  onDragStart={(e) => handleDragStart(photo, e)}
                />
              ))}
            </div>
          )}
