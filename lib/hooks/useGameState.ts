import { useGameStore, GameState } from '@/lib/store/gameStore'
import { useEffect } from 'react'

/**
 * Custom hook for accessing and updating game state
 */
export function useGameState() {
  const store = useGameStore()
  
  return {
    // State
    caseId: store.caseId,
    sessionId: store.sessionId,
    currentStage: store.currentStage,
    discoveredFacts: store.discoveredFacts,
    chatHistory: store.chatHistory,
    theoryHistory: store.theoryHistory,
    unlockedContent: store.unlockedContent,
    revealedContent: store.revealedContent,
    viewedDocuments: store.viewedDocuments,
    isCompleted: store.isCompleted,
    isSolvedCorrectly: store.isSolvedCorrectly,
    isGameCompleted: store.isGameCompleted,
    gameStatus: store.gameStatus,
    hasReadVeronicaLetter: store.hasReadVeronicaLetter,
    hasSeenBlackmailCommentary: store.hasSeenBlackmailCommentary,
    actIViewedClues: store.actIViewedClues,
    actIIPhase1ViewedClues: store.actIIPhase1ViewedClues,
    actIIPhase2ViewedClues: store.actIIPhase2ViewedClues,
    actIIPhase3ViewedClues: store.actIIPhase3ViewedClues,
    actIIPhase3AViewedClues: store.actIIPhase3AViewedClues,
    actIIPhase3BViewedClues: store.actIIPhase3BViewedClues,
    finalPhaseWhoViewedClues: store.finalPhaseWhoViewedClues,
    finalPhaseMotiveViewedClues: store.finalPhaseMotiveViewedClues,
    finalPhaseWhereViewedClues: store.finalPhaseWhereViewedClues,
    tutorialCompleted: store.tutorialCompleted,
    tutorialStep: store.tutorialStep,
    tutorialStarted: store.tutorialStarted,
    tutorialDismissedAt: store.tutorialDismissedAt,
    checklistProgress: store.checklistProgress,
    isLoading: store.isLoading,
    isSyncing: store.isSyncing,
    
    // Actions
    initializeGame: store.initializeGame,
    setSessionId: store.setSessionId,
    setCurrentStage: store.setCurrentStage,
    addDiscoveredFact: store.addDiscoveredFact,
    addChatMessage: store.addChatMessage,
    addTheorySubmission: store.addTheorySubmission,
    unlockSuspect: store.unlockSuspect,
    unlockScene: store.unlockScene,
    unlockRecord: store.unlockRecord,
    revealSuspect: store.revealSuspect,
    revealScene: store.revealScene,
    markDocumentAsViewed: store.markDocumentAsViewed,
    completeGame: store.completeGame,
    markGameAsCompleted: store.markGameAsCompleted,
    resetGame: store.resetGame,
    markLetterAsRead: store.markLetterAsRead,
    markBlackmailCommentaryAsSeen: store.markBlackmailCommentaryAsSeen,
    addActIClue: store.addActIClue,
    addActIIPhase1Clue: store.addActIIPhase1Clue,
    addActIIPhase2Clue: store.addActIIPhase2Clue,
    addActIIPhase3Clue: store.addActIIPhase3Clue,
    addActIIPhase3AClue: store.addActIIPhase3AClue,
    addActIIPhase3BClue: store.addActIIPhase3BClue,
    addFinalPhaseWhoClue: store.addFinalPhaseWhoClue,
    addFinalPhaseMotiveClue: store.addFinalPhaseMotiveClue,
    addFinalPhaseWhereClue: store.addFinalPhaseWhereClue,
    startTutorial: store.startTutorial,
    setTutorialStep: store.setTutorialStep,
    completeTutorial: store.completeTutorial,
    skipTutorial: store.skipTutorial,
    dismissTutorial: store.dismissTutorial,
    resumeTutorial: store.resumeTutorial,
    updateChecklistProgress: store.updateChecklistProgress,
    setLoading: store.setLoading,
    setSyncing: store.setSyncing,
    fetchGameState: store.fetchGameState,
  }
}

/**
 * Hook to initialize game on mount
 */
export function useInitializeGame(caseId: string) {
  const { initializeGame, caseId: currentCaseId } = useGameState()
  
  useEffect(() => {
    // Only initialize if this is a new case
    if (currentCaseId !== caseId) {
      initializeGame(caseId).catch(error => {
        console.error('Failed to initialize game:', error)
      })
    }
  }, [caseId, currentCaseId, initializeGame])
}

/**
 * Hook to get specific chat history for a suspect
 */
export function useSuspectChatHistory(suspectId: string) {
  const { chatHistory } = useGameState()
  return chatHistory.filter(msg => msg.suspectId === suspectId)
}

/**
 * Hook to check if content is unlocked
 */
export function useIsUnlocked(type: 'suspect' | 'scene' | 'record', id: string) {
  const { unlockedContent } = useGameState()
  
  switch (type) {
    case 'suspect':
      return unlockedContent.suspects.has(id)
    case 'scene':
      return unlockedContent.scenes.has(id)
    case 'record':
      return unlockedContent.records.has(id)
    default:
      return false
  }
}

/**
 * Hook to get facts from a specific source
 */
export function useFactsBySource(source: 'chat' | 'record' | 'scene' | 'clue') {
  const { discoveredFacts } = useGameState()
  return discoveredFacts.filter(fact => fact.source === source)
}

